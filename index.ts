...
let enmeysName: string[] = ["Skelton", "Zombie", "Warrior", "Assasin"];
...
Game: while (running) {
  ...
  let enemies: string = enmeysName[Math.floor(Math.random() * (4 - 0) + 0)];
  ...
  while (enemyHealth > 0) {
    ...
    if (userChoice == "Attack") {
      let damageDealt = Math.floor(
        Math.random() * (playerAttackDamage - 0) - 0
      );
      ...
      if (health <= 0) {
        console.log(
          `\t # You have taken too much damage you too weak to go on`
        );
        break Game;
      }
      ...
      if (numHealthPotion > 0) {
        ...
        numHealthPotion--;
        console.log(
          `# You drink the Potion ,healing yourself for${healthPotionHealAmount} \n \t now you have ${health} HP !#  \n\t you have ${numHealthPotion} health potion left`
        );
      } else {
        console.log(
          ` # You have no health Potion left, Defeat enemies for chance to get one`
        );
      }
